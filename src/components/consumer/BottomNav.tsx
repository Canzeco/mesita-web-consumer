...
// Four top-level surfaces plus a fifth Filters cell. Filters is a button,
// not a route — it opens the discovery filter sheet (FilterSheet) over the
// current surface. Pay & Post carries a `primary` flag — its
// icon renders inside a pink-tinted ring-circle even when the tab
// isn't selected so it reads as the lead CTA among the row
    ...
    <>
      <nav className="border-border bg-card/95 z-40 shrink-0 border-t px-1 pt-2 backdrop-blur">
        <div className="flex items-end justify-around">
          ...
          {/* Fifth cell — Filters. A button (not a route) that opens the
              filter sheet over whatever surface you're on. Mirrors the tab
              cell layout so the row stays even and keeps Pay centered. */}
          <button
            type="button"
          ...
          >
            ...
            <span
              className={cn(
                "flex h-8 w-8 items-center justify-center rounded-full transition",
                filtersOpen ? "bg-primary/10 ring-primary/20 ring-1" : "",
              )}
            >
              <SlidersHorizontal
                className="h-5 w-5"
                strokeWidth={filtersOpen ? 2.25 : 1.75}
              />
            </span>
            <span className="w-full truncate text-center">Filters</span>
          </button>
        </div>
