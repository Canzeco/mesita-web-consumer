...
- Five bottom tabs (Home/Search/Rewards/Reservations/Profile). Consumers have a **class** (`/me/class`; `/me/plan` redirects). Favorites = localStorage (`useSavedPlaces()`), not an EF. Referral page = `/share` (`/invite` redirects). AI persona = **Don Memo** (`/discover/ai`, Spanish-first — only the AI's own messages are Spanish).
